...
@Duncan make a note somewhere about Xpath syntax? Suggest a reference? Make a subsection for getting specific nodes?
...
We can extract the text belonging to a given section. By default the section headers are the same as those found by `findSectionHeaders`, so if that function missed any section headers then multiple sections may be grouped by getSectionText.

```
secText = getSectionText(doc, maxNumPages = 50, cleanSectionNums = F)
```

To view sections as text blocks rather than individual lines we can paste sections together by

```
lapply(secText, paste, collapse = " ")
```

To extract the text from a specific section we can combine the functionality of `getNodeSet`with `getSectionText`, specifying in `getNodeSet` the nodes we are interested in and using them as the argument to `secHeaders` in `getSectionText`. For example, if we only want the text from the section containing "Motivation".

**FIX? The text below is a mix of the beginning of the Motivation section with the abstract tacked on.**
```
