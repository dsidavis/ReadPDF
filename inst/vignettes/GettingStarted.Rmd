...
We'll use the RClang.pdf and RClang.xml documents included in the package:
```
...
`getBBox`, e.g.,
```
bb = getBBoxText(doc[[1]])
```


## Font Information

We can a list of all of the fonts used within a document or defined within a page with 
```
fi = getFontInfo(doc)
```
...
`OAZGEE+CMBXSL10` indicates bold extended slanted, while CMBXTI indicates bold extended and italic.
...
We can see that Motivation is the first section title.
...
So we could look for that exact string, or look for a text node that contains Motivation:
```
...
```
since we previously computed the documents font table and assigned it to `fi`.

However, we don't even need the font information, but only its id to find other text nodes
that have the same font.
```
tt = getNodeSet(doc, sprintf("//text[@font = '%s']", id))
```
```
[[1]]
<text top="825" left="379" width="146" height="19" font="16" rotation="0.000000">1. Motivation</text> 
...
The word `RCIndex`  is in a different font. But we can assemble the
title by finding all `<text>` nodes on the same line.
We can do it in this context with
```
getNodeSet(doc[[6]], ".//text[@top = 371]")
```
and we get all the terms in the title and in the correct order.
```
[[1]]
<text top="371" left="325" width="71" height="19" font="16" rotation="0.000000">3. The</text> 
...
Looking at the two primary fonts from these nodes,
```
fi[ c("15", "20"), ]
```
```
...
```
bb = getBBox(doc[[5]], TRUE)
```
The result is a data.frame. This is because we specified `TRUE` for the `asDataFrame` parameter
in the call so that we didn't get a matrix.
The data frame contains the same information as the matrix but has allows for additional
...
Note that isCenetered()  works heuristically and can be fooled.
...
There we knew one title and found the others based on matching this known one.
findSectionHeaders() attempts to find the first one using a variety of heuristics
+ recognize common phrases in titles (e.g. Background, Methods, References)
...
Of course PDF documents can contain images.
The conversion to an XML document does preserve some information about each image
...
so these are included in the data.frame.
The page number is also included by default but can be avoided via the `addPageNum` parameter.

Some images have more information such as the original filename of the image when they were included in the PDF,
a title, a date it was created (again separately from the PDF). In XML, these are additional attributes.
